...
void Border::stop(void) {

}

/** Initialize server resources. */
void Border::start(void) {
	unsigned int i;

	for (i = 0; i < BI_COUNT; i++) {
		if (buttonNames[i]) {
			buttonIcons[i] = Icons::LoadNamedIcon(buttonNames[i], 1, 1);
	...
	if (buttonIcons[BI_MENU] == NULL) {
		buttonIcons[BI_MENU] = Icons::GetDefaultIcon();
	}
}

/** Destroy structures. */
void Border::destroy(void) {
	unsigned i;
	for (i = 0; i < BI_COUNT; i++) {
		if (buttonNames[i]) {
			Release(buttonNames[i]);
...
int Border::GetBorderIconSize(void) {
...
char Border::IsContextEnabled(MouseContextType context, const ClientNode *np) {
	...
	case MC_MOVE:
		return (flags & BORDER_MOVE) != 0;
	case MC_CLOSE:
		return (flags & BORDER_CLOSE) != 0;
	case MC_MAXIMIZE:
		return (flags & BORDER_MAX) != 0;
	case MC_MINIMIZE:
		return (flags & BORDER_MIN) != 0;
	case MC_ICON:
		return 1;
	default:
		return 0;
...
MouseContextType Border::GetBorderContext(const ClientNode *np, int x, int y) {
	...
	if ((np->getState()->getBorder() & BORDER_TITLE)
		...
		if (y >= south && y <= titleHeight + south) {
			...
			while (settings.titleBarLayout[index]) {
				...
				if (nextOffset >= np->getWidth() - west) {
	...
	if (y > np->getHeight() + north - titleHeight) {
		if (x < titleHeight) {
			return (MC_BORDER_S | MC_BORDER_W | MC_BORDER) & resizeMask;
		} else if (x > np->getWidth() + west - titleHeight) {
	...
	} else if (x >= np->getWidth() + west) {
		return (MC_BORDER_E | MC_BORDER) & resizeMask;
	} else if (y >= np->getHeight() + north) {
...
void Border::ResetBorder(const ClientNode *np) {
	...
	if (np->getParent() == None) {
		JXMoveResizeWindow(display, np->getWindow(), np->getX(), np->getY(),
	...
	JXMoveResizeWindow(display, np->getParent(), np->getX() - west,
  ...
  if (settings.cornerRadius > 0
      || (np->getState()->isShaped())) {
		...
		if (!(np->getState()->isShaded())
			...
			rects = JXShapeGetRectangles(display, np->getWindow(),
		...
		/* Set the shape. */
		JXShapeCombineMask(display, np->getParent(), ShapeBounding, 0, 0,
...
void Border::DrawBorderHelper(const ClientNode *np) {
	...
	const int titleHeight = Border::GetTitleHeight();
	...
	if (np->getState()->isStatus(STAT_ACTIVE | STAT_FLASH)) {
		...
		titleColor2 = Colors::lookupColor(COLOR_TITLE_ACTIVE_BG2);
		outlineColor = Colors::lookupColor(COLOR_TITLE_ACTIVE_DOWN);

	} else {

		borderTextColor = COLOR_TITLE_FG;
		titleColor1 = Colors::lookupColor(COLOR_TITLE_BG1);
		titleColor2 = Colors::lookupColor(COLOR_TITLE_BG2);
		outlineColor = Colors::lookupColor(COLOR_TITLE_DOWN);

	}

	/* Set parent background to reduce flicker. */
	JXSetWindowBackground(display, np->getParent(), titleColor2);

	canvas = JXCreatePixmap(display, np->getParent(), width, north, rootDepth);
	...
	if ((np->getState()->getBorder() & BORDER_TITLE)
		...
		/* Draw a title bar. */
		DrawHorizontalGradient(canvas, gc, titleColor1, titleColor2, 0, 1,
				width, titleHeight - 2);
		...
		if (np->getName() && np->getName()[0] && point.x < point.y) {
			...
			const int textWidth = Fonts::GetStringWidth(FONT_BORDER,
					np->getName());
			...
			Fonts::RenderString(canvas, FONT_BORDER, borderTextColor, titlex,
					titley, titleWidth, np->getName());
		}
	...
	if (settings.windowDecorations == DECO_MOTIF) {
		const int off = 2;
		JXCopyArea(display, canvas, np->getParent(), gc, off, off,
				width - 2 * off, north - off, off, off);
		JXClearArea(display, np->getParent(), off, north, width - 2 * off,
				height - north - off, False);
	} else {
		JXCopyArea(display, canvas, np->getParent(), gc, 1, 1, width - 2,
				north - 1, 1, 1);
		JXClearArea(display, np->getParent(), 1, north, width - 2,
				height - north - 1, False);
	}
	...
	if (settings.windowDecorations == DECO_MOTIF) {
		...
		if (np->getState()->isShaded()) {
			...
			DrawRoundedRectangle(np->getParent(), gc, 0, 0, width - 1,
					height - 1, settings.cornerRadius);
		}
	}
...
void Border::DrawBorderHandles(const ClientNode *np, Pixmap canvas, GC gc) {
	...
	if (np->getState()->isStatus(STAT_ACTIVE | STAT_FLASH)) {
		...
		pixelUp = Colors::lookupColor(COLOR_TITLE_UP);
...
char Border::DrawBorderIcon(BorderIconType t, unsigned xoffset,
		unsigned yoffset, Pixmap canvas, long fg) {
	if (buttonIcons[t]) {
#ifdef USE_ICONS
		const unsigned titleHeight = GetTitleHeight();
		Icons::PutIcon(buttonIcons[t], canvas, fg, xoffset + 2, yoffset + 2,
...
/** Draw a close button. */
void Border::DrawCloseButton(unsigned xoffset, unsigned yoffset, Pixmap canvas,
		GC gc, long fg) {
	XSegment segments[2];
	const unsigned titleHeight = GetTitleHeight();
	unsigned size;
	...
	JXSetLineAttributes(display, gc, 2, LineSolid, CapProjecting, JoinBevel);
	JXDrawSegments(display, canvas, gc, segments, 2);
	JXSetLineAttributes(display, gc, 1, LineSolid, CapNotLast, JoinMiter);

}

/** Draw an inactive maximize button. */
void Border::DrawMaxIButton(unsigned xoffset, unsigned yoffset, Pixmap canvas,
		GC gc, long fg) {
	...
	JXSetLineAttributes(display, gc, 1, LineSolid, CapProjecting, JoinMiter);
...
void Border::DrawMaxAButton(unsigned xoffset, unsigned yoffset, Pixmap canvas,
		GC gc, long fg) {
	XSegment segments[8];
	unsigned titleHeight;
	unsigned size;
...
void Border::DrawMinButton(unsigned xoffset, unsigned yoffset, Pixmap canvas,
		GC gc, long fg) {
	unsigned titleHeight;
	unsigned size;
	unsigned x1, y1;
	...
	x2 = x1 + size;
	y2 = y1 + size;
	JXSetLineAttributes(display, gc, 2, LineSolid, CapProjecting, JoinMiter);
...
void Border::DrawIconButton(const ClientNode *np, int x, int y, Pixmap canvas,
	...
	Icons::PutIcon(icon, canvas, fg, x + (titleHeight - iconSize) / 2,
...
void Border::GetBorderSize(const ClientState *state, int *north, int *south,
	...
	if (state->getBorder() & BORDER_OUTLINE) {

		if (state->getBorder() & BORDER_TITLE) {
...
void Border::DrawRoundedRectangle(Drawable d, GC gc, int x, int y, int width,
...
#ifdef USE_SHAPE
void Border::FillRoundedRectangle(Drawable d, GC gc, int x, int y, int width,
		int height, int radius) {
...
void Border::SetBorderIcon(BorderIconType t, const char *name) {
	if (buttonNames[t]) {
