...
static const struct {
	const FontType src;
	const FontType dest;
} INHERITED_FONTS[] = { { FONT_TRAY, FONT_PAGER }, { FONT_TRAY, FONT_CLOCK }, {
		FONT_TRAY, FONT_TASKLIST }, { FONT_TRAY, FONT_TRAYBUTTON } };
...
void Fonts::InitializeFonts(void) {
#ifdef USE_ICONV
   const char *codeset;
#endif
	unsigned int x;
...
char* Fonts::ConvertFromUTF8(char *str) {
...
int Fonts::GetStringWidth(FontType ft, const char *str) {
#ifdef USE_XFT
	XGlyphInfo extents;
#endif
#ifdef USE_FRIBIDI
...
#ifdef USE_XFT
	JXftTextExtentsUtf8(display, fonts[ft], (const unsigned char* )output, len,
			&extents);
	result = extents.xOff;
#else
...
int Fonts::GetStringHeight(FontType ft) {
	Assert(fonts[ft]);
	return fonts[ft]->ascent + fonts[ft]->descent;
}

/** Set the font to use for a component. */
void Fonts::SetFont(FontType type, const char *value) {
	if (JUNLIKELY(!value)) {
...
void Fonts::RenderString(Drawable d, FontType font, ColorType color, int x,
   ...
   rect.width = XTextWidth(fonts[font], output, len);
	...
	JXftDrawStringUtf8(xd, Colors::GetXftColor(color), fonts[font], x,
			y + fonts[font]->ascent, (const unsigned char* )output, len);
