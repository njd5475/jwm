  ...
  if (np) {
    ...
    if (np) {
      const char move_resize =
          (np->getState()->isDragable())
              || ((mask == settings.moveMask)
                  && !(np->getState()->isNotDraggable()));
      switch (event->button) {
      case Button1:
      case Button2:
        np->FocusClient();
    ...
    if (np) {
      if (keyAction) {
        ...
        const unsigned bsize =
            (np->getState()->getBorder() & BORDER_OUTLINE) ?
...
void _HandleNetMoveResize(const XClientMessageEvent *event, ClientNode *np) {
  ...
  if (JUNLIKELY(np->getState()->isFullscreen())) {
    np->SetClientFullScreen(0);
  }
...
void _HandleNetWMState(const XClientMessageEvent *event, ClientNode *np) {
  ...
  case 0: /* Remove */
    ...
    if (actionNolist && !(np->getState()->shouldIgnoreSpecifiedList())) {
      np->clearToNoList();
      _RequireTaskUpdate();
    }
    if (actionNopager && !(np->getState()->shouldIgnorePager())) {
      np->clearToNoPager();
      _RequirePagerUpdate();
    }
...
void _HandleMapRequest(const XMapEvent *event) {
  ...
  if (!np) {
    ...
    if (np) {
      if (!(np->getState()->notFocusableIfMapped())) {
        np->FocusClient();
      }
    } else {
...
void _HandleUnmapNotify(const XUnmapEvent *event) {
  ...
  if (np) {
    ...
    if (JXCheckTypedWindowEvent(display, np->getWindow(), DestroyNotify, &e)) {
      _UpdateTime(&e);
      np->RemoveClient();
    } else if ((np->getState()->isMapped()) || event->send_event) {
