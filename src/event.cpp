...
public:
	...
	virtual ~ConfigureRequestHandler() {
	}

	virtual bool process(const XEvent *event) {
		Events::_HandleConfigureRequest(&event->xconfigurerequest);
		return true;
	}
};
...
HashMap<std::vector<EventHandler*>*> Events::_handlerMap;

char Events::restack_pending = 0;
char Events::task_update_pending = 0;
char Events::pager_update_pending = 0;

ConfigureRequestHandler configReqHandler;

void Events::init() {
	Events::registerHandler(ConfigureRequest, &configReqHandler);
}
...
//        Roots::Restart();
		...
		if (_handlerMap.hasKey(eventName)) {
			for (auto handler : *_handlerMap[eventName].value()) {
...
//  }
	...
	if (np) {
		...
		// if the event value is not zero and height is different
		...
		if (atom == XA_WM_NAME) {
			...
			if (event->atom == Hints::atoms[ATOM_WM_COLORMAP_WINDOWS]) {
				...
				changed = 1;
					...
					== Hints::atoms[ATOM_NET_WM_WINDOW_OPACITY]) {
	...
	if (np) {
		if (event->message_type == Hints::atoms[ATOM_WM_CHANGE_STATE]) {
			...
			default:
				...
				== Hints::atoms[ATOM_NET_ACTIVE_WINDOW]) {
			...
			_HandleNetMoveResize(event, np);
				...
				== Hints::atoms[ATOM_NET_WM_MOVERESIZE]) {

			HandleNetWMMoveResize(event, np);

		} else if (event->message_type
				== Hints::atoms[ATOM_NET_RESTACK_WINDOW]) {
...
//      Roots::ReloadMenu();
			Log("NOT IMPLEMENTED: Should reload menu");
				...
				== Hints::atoms[ATOM_NET_CURRENT_DESKTOP]) {
			DesktopEnvironment::DefaultEnvironment()->ChangeDesktop(
					event->data.l[0]);
		} else if (event->message_type
				== Hints::atoms[ATOM_NET_SHOWING_DESKTOP]) {
			DesktopEnvironment::DefaultEnvironment()->ShowDesktop();
		} else {
#ifdef DEBUG
			atomName = JXGetAtomName(display, event->message_type);
...
void Events::registerUnconsumedHandler(EventHandler *handler) {
	handlers.push_back(handler);
}

void Events::registerHandler(const int eventType, EventHandler *handler) {
	std::vector<EventHandler*> *events;
	const char *eventName = Events::getEventName(eventType);
	if (_handlerMap.hasKey(eventName)) {
		events = _handlerMap[eventName].value();
	} else {
		events = new std::vector<EventHandler*>;
		_handlerMap[eventName] = events;
	}
