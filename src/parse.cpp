...
static const StringMappingType MAP[] = { { "close", CLOSE }, { "ddesktop", DDESKTOP }, { "desktop#", DESKTOP }, {
    ...
    "rdesktop", RDESKTOP }, { "resize", RESIZE }, { "restart", RESTART }, { "restore", RESTORE }, { "right", RIGHT }, {
    "select", ENTER }, { "send#", SEND }, { "sendd", SENDD }, { "sendl", SENDL }, { "sendr", SENDR },
    { "sendu", SENDU }, { "shade", SHADE }, { "showdesktop", SHOWDESK }, { "showtray", SHOWTRAY }, { "stick", STICK }, {
        "udesktop", UDESKTOP }, { "up", UP }, { "window", WIN } };
static const unsigned int MAP_COUNT = ARRAY_LENGTH(MAP);

/** Mapping of key names to key types.
 * Note that this mapping must be sorted.
 */
static const StringMappingType MC_MAP[] = { { "border", MC_BORDER }, { "close", MC_CLOSE }, { "icon", MC_ICON }, {
    "maximize", MC_MAXIMIZE }, { "minimize", MC_MINIMIZE }, { "root", MC_ROOT }, { "title", MC_MOVE } };
...
void ParseSnapMode(const TokenNode *tp) {
  const char *distance;
  static const StringMappingType mapping[] = { { "border", SNAP_BORDER }, { "none", SNAP_NONE },
      { "screen", SNAP_SCREEN } };
...
StatusWindowType ParseStatusWindowType(const TokenNode *tp) {
  static const StringMappingType mapping[] = { { "corner", SW_CORNER }, { "off", SW_OFF }, { "screen", SW_SCREEN }, {
      "window", SW_WINDOW } };
  return ParseAttribute(mapping, ARRAY_LENGTH(mapping), tp, "coordinates", SW_SCREEN);
}
