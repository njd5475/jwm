...
static const StringMappingType MAP[] = { { "close", CLOSE }, { "ddesktop",
    ...
    { "move", MOVE }, { "next", NEXT }, { "nextstacked", NEXTSTACK }, { "none",
        ...
        RDESKTOP }, { "resize", RESIZE }, { "restart", RESTART }, { "restore",
        RESTORE }, { "right", RIGHT }, { "select", ENTER }, { "send#", SEND }, {
        "sendd", SENDD }, { "sendl", SENDL }, { "sendr", SENDR }, { "sendu",
        SENDU }, { "shade", SHADE }, { "showdesktop", SHOWDESK }, { "showtray",
        SHOWTRAY }, { "stick", STICK }, { "udesktop", UDESKTOP }, { "up", UP },
    { "window", WIN } };
...
MC_CLOSE }, { "icon", MC_ICON }, { "maximize", MC_MAXIMIZE }, { "minimize",
    MC_MINIMIZE }, { "root", MC_ROOT }, { "title", MC_MOVE } };
...
static const StringMappingType OPTION_MAP[] = { { "aerosnap", OPTION_AEROSNAP },
    { "border", OPTION_BORDER }, { "centered", OPTION_CENTERED }, { "constrain",
        OPTION_CONSTRAIN }, { "drag", OPTION_DRAG }, { "fixed",
    OPTION_FIXED }, { "fullscreen", OPTION_FULLSCREEN },
    { "hmax", OPTION_MAX_H }, { "iignore",
    OPTION_IIGNORE }, { "ilist", OPTION_ILIST }, { "ipager", OPTION_IPAGER }, {
        "maximized", OPTION_MAXIMIZED }, { "minimized", OPTION_MINIMIZED }, {
        "noborder", OPTION_NOBORDER }, { "noclose", OPTION_NOCLOSE }, {
        "nodrag",
        OPTION_NODRAG }, { "nofocus", OPTION_NOFOCUS }, { "nofullscreen",
        OPTION_NOFULLSCREEN }, { "nolist",
...
template<class T>
static T findOrDefault(const TokenNode *tok, const char *attrName,
    T defaultVal);
...
void ParseActive(const TokenNode *tp, ColorName fg, ColorName bg1,
    ColorName bg2, ColorName up, ColorName down) {
  const TokenNode *np;
...
void ParsePager(const TokenNode *tp, Tray *tray) {
  ...
  PagerType *pager = PagerType::CreatePager(labeled, tray,
...
void ParseSwallow(const TokenNode *tp, Tray *tray) {
  ...
  cp = new SwallowNode(name, tp->value, width, height, tray,
      tray->getLastComponent());
  tray->AddTrayComponent(cp);
...
void ParseTrayButton(const TokenNode *tp, Tray *tray) {
  ...
  cp = TrayButton::Create(icon, label, popup, width, height, tray,
      tray->getLastComponent());
  if (JLIKELY(cp)) {
    tray->AddTrayComponent(cp);
...
void ParseBattery(const TokenNode *tp, Tray *tray) {
  ...
  TrayComponent *cp = new Battery(width, height, tray,
      tray->getLastComponent());
  tray->AddTrayComponent(cp);
}
...
void ParseClock(const TokenNode *tp, Tray *tray) {
  ...
  ClockType *clock = ClockType::CreateClock(format, zone, width, height, tray,
      tray->getLastComponent());
...
void ParseDock(const TokenNode *tp, Tray *tray) {
  ...
  TrayComponent *cp = DesktopEnvironment::DefaultEnvironment()->CreateDock(
      width, tray, tray->getLastComponent());
  tray->AddTrayComponent(cp);
...
void ParsePopupStyle(const TokenNode *tp) {
  static const StringMappingType enable_mapping[] = {
      ...
      POPUP_NONE }, { "menu", POPUP_MENU }, { "pager", POPUP_PAGER }, { "task",
          POPUP_TASK }, { "true",
      POPUP_ALL } };
  const TokenNode *np;
  const char *str;
  ...
  if (tok) {
    ...
    tok = strtok(tok, ",");
    while (tok) {
      const int x = FindValue(enable_mapping, ARRAY_LENGTH(enable_mapping),
          tok);
      if (JLIKELY(x >= 0)) {
...
StatusWindowType ParseStatusWindowType(const TokenNode *tp) {
  static const StringMappingType mapping[] = { { "corner", SW_CORNER }, { "off",
      SW_OFF }, { "screen", SW_SCREEN }, { "window", SW_WINDOW } };
