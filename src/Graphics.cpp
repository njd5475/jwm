...
void Graphics::print(const char *text, int x, int y, int textWidth) {
  Fonts::RenderString(surface, FONT_MENU, COLOR_MENU_FG, x, y, textWidth, text);
}
...
void Graphics::copy(Drawable dest, int srcX, int srcY, int width, int height,
    int destX, int destY) {
  JXCopyArea(_display, surface, dest, context, srcX, srcY, width, height, destX,
      destY);
}
...
void Graphics::drawButton(ButtonType type, AlignmentType alignment,
    FontType font, const char *text, bool fill, bool border, struct Icon *icon,
    int x, int y, int width, int height, int xoffset, int yoffset) {
  DrawButton(type, alignment, font, text, fill, border, this->surface, icon, x,
      y, width, height, xoffset, yoffset);
}

std::vector<Graphics*> Graphics::graphics;

Graphics* Graphics::create(Display *display, GC gc, Drawable d, int width,
    int height, int rootDepth) {
