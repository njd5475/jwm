...
void ActionNode::ProcessActionPress(struct TrayComponentType *cp, int x, int y, int button) {
  const ScreenType *sp;
...
void ActionNode::ProcessActionRelease(struct TrayComponentType *cp, int x, int y, int button) {
  const int mask = 1 << button;
...
void ActionNode::ValidateAction() {
  if (this->action && !strncmp(this->action, "root:", 5)) {
    ...
    if (JUNLIKELY(!IsRootMenuDefined(bindex))) {
      Warning(_("action: root menu \"%s\" not defined"), &this->action[5]);
    }
  }
}
