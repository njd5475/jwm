...
void Backgrounds::_InitializeBackgrounds(void) {
  backgrounds = NULL;
...
void Backgrounds::_StartupBackgrounds(void) {
...
void Backgrounds::_ShutdownBackgrounds(void) {
  BackgroundNode *bp;
  for (bp = backgrounds; bp; bp = bp->next) {
    if (bp->pixmap != None) {
...
void Backgrounds::_DestroyBackgrounds(void) {
  BackgroundNode *bp;
  while (backgrounds) {
    bp = backgrounds->next;
...
void Backgrounds::_SetBackground(int desktop, const char *type,
    const char *value) {
  static const StringMappingType mapping[] = {
      { "command", BACKGROUND_COMMAND }, { "gradient", BACKGROUND_GRADIENT }, {
          "image", BACKGROUND_STRETCH }, { "scale", BACKGROUND_SCALE }, {
          "solid", BACKGROUND_SOLID }, { "tile", BACKGROUND_TILE } };
...
void Backgrounds::_LoadBackground(int desktop) {
  ...
  if (lastBackground && bp->type == lastBackground->type
...
void LoadGradientBackground(BackgroundNode *bp) {
  ...
  if (color1.pixel == color2.pixel) {
    ...
    bp->pixmap = JXCreatePixmap(display, rootWindow, 1, rootHeight, rootDepth);
    DrawHorizontalGradient(bp->pixmap, rootGC, color1.pixel, color2.pixel, 0, 0,
        1, rootHeight);
  }
