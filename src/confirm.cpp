...
DialogsEventHandler Dialogs::handler;
...
bool DialogsEventHandler::process(const XEvent *event) {
  Assert(event);
...
void ExposeConfirmDialog(void) {
  Assert(dialog);
  JXCopyArea(display, dialog->pmap, dialog->node->getWindow(), rootGC, 0, 0,
      dialog->width, dialog->height, 0, 0);
}
...
void DrawDialog(void) {
  ...
  JXFillRectangle(display, dialog->pmap, rootGC, 0, 0, dialog->width,
      dialog->height);

  /* Draw the message. */
  yoffset = 4;
  for (x = 0; x < dialog->lineCount; x++) {
    Fonts::RenderString(dialog->pmap, FONT_MENU, COLOR_MENU_FG, 4, yoffset,
        dialog->width, dialog->message[x]);
...
void DrawButtons(void) {
  ...
  Icon *icon = NULL;
  DrawButton(type, alignment, font, text, true, border, drawable, icon, x, y,
  ...
  x = dialog->cancelx;
  y = dialog->buttony;
  DrawButton(type, alignment, font, cancelText, true, border, drawable, icon, x,
