				...
				for (cp = entry->clients; cp; cp = cp->next) {
					...
					for (layer = LAST_LAYER; layer >= FIRST_LAYER; layer--) {
						...
						for (np = nodes[layer]; np; np = np->getNext()) {
							...
							if (np == cp->client) {
								const char isActive = (np->getState()->getStatus() & STAT_ACTIVE)
...
void TaskBar::SignalTaskbar(const TimeType *now, int x, int y, Window w, void *data) {
...
TaskBar *TaskBar::bars = NULL;
TaskEntry *TaskBar::taskEntries = NULL;
TaskEntry *TaskBar::taskEntriesTail = NULL;
