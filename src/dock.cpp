...
void Dock::_StartupDock(void) {
  ...
  if (dock->window == None) {
    ...
    snprintf(selectionName, sizeof(BASE_SELECTION_NAME), BASE_SELECTION_NAME,
        rootScreen);
...
Dock::~Dock() {
  ...
  while (!nodes.empty()) {
    node = *nodes.begin();
    JXReparentWindow(display, node->window, rootWindow, 0, 0);
    Release(node);
...
void Dock::Create() {
  ...
  Hints::SetCardinalAtom(dock->getWindow(), ATOM_NET_SYSTEM_TRAY_ORIENTATION,
      orientation);
  ...
  if (!owner) {
    ...
    if (JUNLIKELY(
        JXGetSelectionOwner(display, dockAtom) != dock->getWindow())) {
...
char Dock::_HandleDockResizeRequest(const XResizeRequestEvent *event) {
  ...
  for(int dn = 0; dn < dock->nodes.size(); ++dn) {
    np = dock->nodes[dn];
...
void Dock::_DockWindow(Window win) {
  ...
  np->window = win;
  np->needs_reparent = 0;
  dock->nodes.push_back(np);

  /* Update the requested size. */
  int newWidth = dock->getRequestedWidth(), newHeight =
      dock->getRequestedHeight();
  _GetDockSize(&newWidth, &newHeight);
...
char Dock::_HandleDockDestroy(Window win) {
  ...
  while(!dock->nodes.empty()) {
    DockNode *dp = *dock->nodes.begin();
    if (dp->window == win) {

      /* Remove the window from our list. */
      dock->nodes.erase(dock->nodes.begin());
      Release(dp);

      /* Update the requested size. */
      int newWidth = dock->getRequestedWidth();
      int newHeight = dock->getRequestedHeight();
      _GetDockSize(&newWidth, &newHeight);
      dock->requestNewSize(newWidth, newHeight);
...
void Dock::_UpdateDock(void) {
  ...
  y = 0;
  memset(&event, 0, sizeof(event));
  for(int i = 0; i < dock->nodes.size(); ++i) {
    np = dock->nodes[i];
...
void Dock::_GetDockSize(int *width, int *height) {
  ...
  for(int dn = 0; dn < dock->nodes.size(); ++dn) {
    np = dock->nodes[dn];
    const unsigned spacing = settings.dockSpacing;
    if (orientation == SYSTEM_TRAY_ORIENTATION_HORZ) {
