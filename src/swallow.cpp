...
SwallowNode::SwallowNode(const char *name, const char *command, int width, int height) : TrayComponentType(), border(0) {
  if (JUNLIKELY(!name)) {
  ...
  for (np = swallowNodes; np; np = np->next) {
    if (event->xany.window == np->window) {
      ...
      case DestroyNotify:
        ...
        break;
      case ResizeRequest:
        np->requestedWidth = event->xresizerequest.width + np->border * 2;
        np->requestedHeight = event->xresizerequest.height + np->border * 2;
        np->getTray()->ResizeTray();
        break;
      case ConfigureNotify:
        ...
        if (width != np->getRequestedWidth() && height != np->getRequestedHeight()) {
...
void SwallowNode::Destroy() {
  ...
  if (this->window) {
    ...
    if (state.status & STAT_DELETE) {
      ClientNode::SendClientMessage(this->window, ATOM_WM_PROTOCOLS, ATOM_WM_DELETE_WINDOW);
    } else {
